...
pub struct Bindings
{
    pub(crate) gamepad_button_bindings:
...
impl Bindings
{
    ...
    pub fn merge(&mut self, bindings: Bindings)
    {
        ...
        self.keyboard_key_bindings =
            Bindings::get_merged_hashmaps(self.keyboard_key_bindings.clone(),
                                          bindings.keyboard_key_bindings);
        // mouse
        self.mouse_button_binding =
            Bindings::get_merged_hashmaps(self.mouse_button_binding.clone(),
                                          bindings.mouse_button_binding);
        self.mouse_move_binding =
            Bindings::get_merged_hashmaps(self.mouse_move_binding.clone(),
                                          bindings.mouse_move_binding);
        // actions
        self.action_deadzone =
            Bindings::get_merged_hashmaps(self.action_deadzone.clone(),
                                          bindings.action_deadzone);
        self.action_phase =
            Bindings::get_merged_hashmaps(self.action_phase.clone(),
                                          bindings.action_phase);
    }

    // private
    // src: https://stackoverflow.com/questions/27244465/merge-two-hashmaps-in-rust
    fn get_merged_hashmaps<K: Hash + Eq + Clone, V: Clone>(map1: HashMap<K, V>,
                                                           map2: HashMap<K, V>)
                                                           -> HashMap<K, V>
    {
...
impl Kurinji
{
    ...
    {
        Bindings { gamepad_button_bindings: self.joystick_button_binding
                                                .clone(),
                   gamepad_axis_bindings:   self.joystick_axis_binding.clone(),
                   keyboard_key_bindings:   self.keyboard_action_binding
                                                .clone(),
